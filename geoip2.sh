...
trap '
    ...
    if [ -n "$TEMP_CONF" ] && [ -f "$TEMP_CONF" ]; then
        rm -f "$TEMP_CONF" && log_info "✅ 清理临时配置文件: $TEMP_CONF" || log_warn "⚠️ 清理临时配置文件失败: $TEMP_CONF"
    fi
    # 删除DEFAULT_CONF.new临时文件
    if [ -f "${DEFAULT_CONF}.new" ]; then
        rm -f "${DEFAULT_CONF}.new" && log_info "✅ 清理默认配置临时文件: ${DEFAULT_CONF}.new" || log_warn "⚠️ 清理默认配置临时文件失败: ${DEFAULT_CONF}.new"
    fi
    # 仅删除当前脚本生成的Nginx源码/模块（加$$后缀，进程隔离）
    if [ -n "$NGINX_VERSION" ]; then
        NGINX_TMP_DIR="/tmp/nginx-${NGINX_VERSION}_$$"
        if [ -d "$NGINX_TMP_DIR" ]; then
            rm -rf "$NGINX_TMP_DIR" "/tmp/nginx-$$" && log_info "✅ 清理Nginx源码目录: $NGINX_TMP_DIR" || log_warn "⚠️ 清理Nginx源码目录失败: $NGINX_TMP_DIR"
        fi
        MODULE_TMP_DIR="/tmp/${MODULE_NAME}_$$"
        if [ -d "$MODULE_TMP_DIR" ]; then
            rm -rf "$MODULE_TMP_DIR" && log_info "✅ 清理GeoIP2模块目录: $MODULE_TMP_DIR" || log_warn "⚠️ 清理GeoIP2模块目录失败: $MODULE_TMP_DIR"
        fi
    fi
    # 清理Nginx压缩包（通配符不加引号，正确展开）
    NGINX_TMP_TAR="/tmp/nginx-*.tar.gz_$$"
    if compgen -G "$NGINX_TMP_TAR" > /dev/null; then
        rm -f $NGINX_TMP_TAR && log_info "✅ 清理Nginx压缩包: $NGINX_TMP_TAR" || log_warn "⚠️ 清理Nginx压缩包失败: $NGINX_TMP_TAR"
    fi
    log_info "✅ 脚本退出，临时文件清理完成";
' EXIT INT TERM
mkdir -p /var/log
touch "$LOG_FILE"
chmod 644 "$LOG_FILE"
...
detect_package_manager() {
    if [ -f /etc/debian_version ]; then
        ...
        PM_UPDATE="update -o Acquire::Timeout=60 -y"
...
interactive_config() {
    ...
    while true; do
        ...
        read -p "$(echo -e "${YELLOW}是否使用默认Let's Encrypt证书路径 "/etc/letsencrypt/live/$CUSTOM_DOMAIN/" ？(y/n) [默认: y]${NC}\n选择：")" USE_DEFAULT_CERT
        # 未输入则使用默认y
...
if command -v docker &> /dev/null; then
    ...
    curl -fL --connect-timeout 30 --max-time 60 https://get.docker.com | bash
    if [ $? -ne 0 ]; then
...
if [ "$CONTAINER_EXISTS" -eq 0 ] || [ "$CURRENT_PORT" != "$SPEED_PORT" ]; then
    ...
    if [ "$CONTAINER_EXISTS" -eq 1 ]; then
        ...
        log_info "🗑️️ 已删除旧容器（端口不匹配：旧端口 $CURRENT_PORT → 新端口 $SPEED_PORT）"
    fi
    # 新建容器
    docker run -d \
      --restart always \
    ...
    log_info "✅ LibreSpeed 容器新建完成（端口：$SPEED_PORT）"
else
    # 容器存在且端口匹配 → 仅检查运行状态
    log_info "✅ LibreSpeed 容器已存在且端口匹配，跳过重建"
...
if [ -f /etc/nginx/sites-enabled/default ]; then
    ...
    log_info "✅ 禁用Nginx默认站点配置（Ubuntu 24.04）"
fi
...
if [ -f "$DEFAULT_CONF" ]; then
    rm -f "$DEFAULT_CONF"
    log_info "🗑️️ 已删除旧的 default.conf 配置文件"
fi
# 3.4 下载Nginx源码
...
MODULE_TMP_DIR="/tmp/${MODULE_NAME}_$$"
if [ ! -d "$MODULE_TMP_DIR" ]; then
    git clone "$MODULE_GIT_URL" "$MODULE_TMP_DIR" || {
        log_error "❌ GeoIP2模块克隆失败"
        restore_nginx_config
        exit 1
    }
fi
log_info "✅ GeoIP2模块下载完成: $MODULE_TMP_DIR"
# 3.6 编译动态模块
log_info "5. 编译GeoIP2动态模块..."
cd "$NGINX_SRC_DIR"
./configure --with-compat --add-dynamic-module="$MODULE_TMP_DIR" || {
    log_error "编译配置失败"
    restore_nginx_config
    exit 1
}
...
if [ "$USE_CLOUDFLARE" = true ]; then
    ...
    CF_IPV4=$(curl -s --fail --connect-timeout 30 --max-time 60 https://www.cloudflare.com/ips-v4 2>/dev/null || true)
    CF_IPV6=$(curl -s --fail --connect-timeout 30 --max-time 60 https://www.cloudflare.com/ips-v6 2>/dev/null || true)
...
if [ "$USE_CLOUDFLARE" = true ]; then
    ...
    if ! grep -q "# Cloudflare RealIP Configuration" "$DEFAULT_CONF"; then
        # 生成RealIP配置到临时文件
        cat > "$TEMP_CONF" << EOL
...
if [ -f "$DB_FILE" ]; then
    ...
    # 校验：文件大小≥500KB
    if [ $(stat -c%s "$DB_FILE" 2>/dev/null || echo 0) -ge 512000 ]; then
...
if [ ! -f "$DB_FILE" ]; then
    ...
    for url in "${GEOIP_DB_URLS[@]}"; do
        ...
        # 下载文件
        curl -L --connect-timeout 30 --max-time 60 "$url" -o "$DB_FILE"
        # 下载后立即校验（文件存在 + 大小≥4MB）
        if [ -f "$DB_FILE" ] && [ $(stat -c%s "$DB_FILE" 2>/dev/null || echo 0) -ge 4194304 ]; then
            download_success=1
            log_info "✅ 数据库下载并校验成功"
            break  # 校验成功，停止尝试后续源
        else
...
if ! grep -q "server {" "$DEFAULT_CONF"; then
...
# 3.17 清理临时文件
log_info "17. 清理临时文件..."
# 安全清理 Nginx 编译目录（匹配创建时的$$后缀）
if [ -n "$NGINX_VERSION" ]; then
    NGINX_TMP_DIR="/tmp/nginx-${NGINX_VERSION}_$$"
    if [ -d "$NGINX_TMP_DIR" ]; then
        rm -rf "$NGINX_TMP_DIR" "/tmp/nginx-$$" && log_info "✅ 清理Nginx源码目录: $NGINX_TMP_DIR" || log_warn "⚠️ 清理Nginx源码目录失败: $NGINX_TMP_DIR"
    fi
    # 清理Nginx压缩包（匹配创建时的$$后缀）
    NGINX_TMP_TAR="/tmp/nginx-${NGINX_VERSION}.tar.gz_$$"
    if [ -f "$NGINX_TMP_TAR" ]; then
        rm -f "$NGINX_TMP_TAR" && log_info "✅ 清理Nginx压缩包: $NGINX_TMP_TAR" || log_warn "⚠️ 清理Nginx压缩包失败: $NGINX_TMP_TAR"
    fi
fi
# 安全清理 GeoIP2 模块目录（带$$后缀，进程隔离）
MODULE_TMP_DIR="/tmp/${MODULE_NAME}_$$"
if [ -d "$MODULE_TMP_DIR" ]; then
    rm -rf "$MODULE_TMP_DIR" && log_info "✅ 清理GeoIP2模块目录: $MODULE_TMP_DIR" || log_warn "⚠️ 清理GeoIP2模块目录失败: $MODULE_TMP_DIR"
fi
# 清理临时配置文件（兜底：trap已处理，此处仅二次检查）
if [ -n "$TEMP_CONF" ] && [ -f "$TEMP_CONF" ]; then
    rm -f "$TEMP_CONF" && log_info "✅ 清理临时配置文件: $TEMP_CONF" || log_warn "⚠️ 清理临时配置文件失败: $TEMP_CONF"
fi
if [ -f "${DEFAULT_CONF}.new" ]; then
