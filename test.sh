...
echo "========================================"
log_info "✅ 一体化脚本执行完成！"
log_info "✅ 测速端口：$SPEED_PORT"
log_info "✅ 拦截国家：$BLOCKED_COUNTRIES"
log_info "✅ 访问域名：$CUSTOM_DOMAIN"
echo "========================================"
echo ""
log_info "直连访问：http://服务器IP:$SPEED_PORT"
if [ "$SSL_ENABLED" = true ]; then
    log_info "域名访问：https://$CUSTOM_DOMAIN"
elif [ "$CUSTOM_DOMAIN" != "localhost" ]; then
    log_info "域名访问：http://$CUSTOM_DOMAIN"
fi
