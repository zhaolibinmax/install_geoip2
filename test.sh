...
curl -sL --retry 5 --connect-timeout 15 https://nginx.org/download/nginx-$NGINX_VERSION.tar.gz -o nginx-$NGINX_VERSION.tar.gz
tar zxf nginx-$NGINX_VERSION.tar.gz || { log_error "解压失败"; exit 1; }

log_info "4. 下载GeoIP2模块..."
[ ! -d "/tmp/$MODULE_NAME" ] && git clone --depth 1 "$MODULE_GIT_URL" /tmp/$MODULE_NAME

log_info "5. 编译模块..."
cd /tmp/nginx-$NGINX_VERSION || { log_error "源码目录不存在"; exit 1; }
[ ! -d "auto" ] && { log_error "Nginx源码不完整"; exit 1; }
