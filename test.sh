...
# 初始化日志
trap 'rm -f "$TEMP_CONF" "${DEFAULT_CONF}.new" "/tmp/nginx.conf.tmp.*";
    rm -rf "/tmp/nginx-$NGINX_VERSION" "/tmp/$MODULE_NAME" "/tmp/nginx-*.tar.gz";
    log_info "✅ 脚本退出，已清理临时文件";' EXIT INT TERM
mkdir -p /var/log
touch "$LOG_FILE"
chmod 644 "$LOG_FILE"
# 日志重定向
...
interactive_config() {
    ...
    # localhost跳过Cloudflare和证书路径交互的标记
    SKIP_CF_CERT=false
    if [ "$CUSTOM_DOMAIN" = "localhost" ]; then
        SKIP_CF_CERT=true
        USE_CLOUDFLARE=false  # localhost强制不使用Cloudflare
        USE_DEFAULT_CERT=true # localhost强制使用默认证书（后续会跳过SSL配置）
...
if ! command -v nginx &> /dev/null; then
    ...
    $PM update -o Acquire::Timeout=300 -y
    $PM install -y nginx
fi
...
log_info "2. 安装系统依赖..."
$PM update -o Acquire::Timeout=300 -y
$PM install -y build-essential libpcre3-dev zlib1g-dev libmaxminddb-dev git curl libssl-dev
log_info "✅ 依赖安装完成"
...
# 3.4 下载Nginx源码
# 替换原脚本中"3. 准备Nginx源码..."部分的代码
log_info "3. 准备Nginx源码..."
cd /tmp
NGINX_SRC_TAR="nginx-$NGINX_VERSION.tar.gz"
...
if [ ! -f "$NGINX_SRC_TAR" ] || [ ! -d "$NGINX_SRC_DIR" ] || [ ! -f "$NGINX_SRC_DIR/auto/options" ]; then
    ...
    if ! curl -s -L --connect-timeout 10 --max-time 60 https://nginx.org/download/$NGINX_SRC_TAR -o $NGINX_SRC_TAR; then
        ...
        curl -s -L --connect-timeout 10 --max-time 60 https://cdn.jsdelivr.net/gh/nginx/nginx@release-$NGINX_VERSION/$NGINX_SRC_TAR -o $NGINX_SRC_TAR
    fi
    # 解压并校验核心文件
...
if [ ! -f "$GEOIP_DB_PATH/GeoLite2-Country.mmdb" ]; then
    ...
    curl -s -L --connect-timeout 100 --max-time 300 https://github.com/zhaolibinmax/install_geoip2/raw/refs/heads/main/GeoLite2-Country.mmdb \
    -o "$GEOIP_DB_PATH/GeoLite2-Country.mmdb" || {
    log_warn "下载失败，尝试GitHub源1..."
    curl -s -L --connect-timeout 100 --max-time 300  \
    -o "$GEOIP_DB_PATH/GeoLite2-Country.mmdb" || {
