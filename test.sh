...
trap '
    ...
    [ -n "$NGINX_VERSION" ] && rm -rf "/tmp/nginx-${NGINX_VERSION}_$$" "/tmp/${MODULE_NAME}_$$"
...
install_nginx_official_repo() {
    ...
    if [ "$PM" = "apt-get" ]; then
        ...
        if [ -f /etc/debian_version ] && ! grep -q "Ubuntu" /etc/os-release 2>/dev/null; then
            ...
            local actual_fingerprint=$(gpg --dry-run --quiet --no-keyring --import --import-options import-show \
                /usr/share/keyrings/nginx-archive-keyring.gpg 2>/dev/null | grep -oP "$expected_fingerprint" || true)
...
EOF
        ...
        # 重新启用gpgcheck并验证安装
        sed -i 's/gpgcheck=0/gpgcheck=1/' /etc/yum.repos.d/nginx.repo 2>/dev/null || true
    fi
...
detect_package_manager() {
    if [ -f /etc/debian_version ]; then
        PM="apt-get"
        PM_INSTALL="install -y -qq"
        PM_UPDATE="update -o Acquire::Timeout=300 -y -qq"
...
if [ ! -f "$DB_FILE" ]; then
    ...
    for url in "${GEOIP_DB_URLS[@]}"; do
        ...
        # 下载后立即校验（文件存在 + 大小≥5000KB）
        if [ -f "$DB_FILE" ] && [ $(stat -c%s "$DB_FILE" 2>/dev/null || echo 0) -ge 5120000 ]; then
            download_success=1
            log_info "✅ 数据库下载并校验成功"
            break  # 校验成功，停止尝试后续源
        else
