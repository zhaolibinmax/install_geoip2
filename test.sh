...
# ==================== 临时文件清理配置 ====================
# 定义清理函数：删除脚本产生的所有/tmp临时文件
cleanup_temp_files() {
    log_info "===== 开始清理/tmp临时文件 ====="
    # 1. 清理nginx配置临时文件
    if [ -f "$TEMP_CONF" ]; then
        rm -f "$TEMP_CONF"
        log_info "✅ 已删除: $TEMP_CONF"
    fi
    if [ -f "$DEFAULT_CONF_TMP" ]; then
        rm -f "$DEFAULT_CONF_TMP"
        log_info "✅ 已删除: $DEFAULT_CONF_TMP"
    fi
    # 2. 清理Nginx源码压缩包和目录（带$$后缀）
    if [ -n "$NGINX_SRC_TAR" ] && [ -f "/tmp/$NGINX_SRC_TAR" ]; then
        rm -f "/tmp/$NGINX_SRC_TAR"
        log_info "✅ 已删除: /tmp/$NGINX_SRC_TAR"
    fi
    if [ -n "$NGINX_SRC_DIR" ] && [ -d "/tmp/$NGINX_SRC_DIR" ]; then
        rm -rf "/tmp/$NGINX_SRC_DIR"
        log_info "✅ 已删除: /tmp/$NGINX_SRC_DIR"
    fi
    # 3. 清理GeoIP2模块源码目录
    if [ -d "/tmp/$MODULE_NAME" ]; then
        rm -rf "/tmp/$MODULE_NAME"
        log_info "✅ 已删除: /tmp/$MODULE_NAME"
    fi
    # 4. 清理Nginx默认解压的源码目录（防止重命名失败残留）
    if [ -n "$NGINX_SRC_DIR_ORIG" ] && [ -d "/tmp/$NGINX_SRC_DIR_ORIG" ]; then
        rm -rf "/tmp/$NGINX_SRC_DIR_ORIG"
        log_info "✅ 已删除残留: /tmp/$NGINX_SRC_DIR_ORIG"
    fi
    log_info "===== /tmp临时文件清理完成 ====="
}
# 设置退出陷阱：无论正常退出/异常退出，都执行清理函数
...
interactive_config() {
    ...
    if [ -n "$INPUT_BLOCK" ]; then
        ...
        if [ -n "$processed_input" ]; then
            ...
            log_info "✅ 已设置拦截国家：$BLOCKED_COUNTRIES"
        else
            log_warn "⚠️ 输入的国家代码无效，使用默认值：CN"
        fi
    else
        log_info "✅ 使用默认拦截国家：CN"
    fi
    echo ""
    # 3. 自定义SSL证书路径
    while true; do
...
if docker ps -a --format "{{.Names}}" | grep "^librespeed$"; then
    ...
    # 【优化3】容器不存在，全新创建，添加安全提示
    log_info "🔒 安全提示：LibreSpeed 将绑定到 127.0.0.1，仅允许本地 Nginx 反向代理访问"
    docker run -d \
      --restart always \
      --name librespeed \
...
log_info "✅ LibreSpeed 部署成功（仅本地可访问，通过 Nginx 反向代理对外提供服务）"
echo ""
...
EOL
        ...
        # 【优化1】将新配置插入到DEFAULT_CONF开头，使用统一临时文件
...
echo "}" >> "$TEMP_CONF"
echo "" >> "$TEMP_CONF"
# 【优化1】将GeoIP2配置插入到DEFAULT_CONF开头，使用统一临时文件
cat "$TEMP_CONF" "$DEFAULT_CONF" > "$DEFAULT_CONF_TMP"
mv "$DEFAULT_CONF_TMP" "$DEFAULT_CONF"
rm -f "$TEMP_CONF"
